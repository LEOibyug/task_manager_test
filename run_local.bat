...
cd /d "%ROOT_DIR%"
"%VENV_PYTHON%" -m uvicorn app.main:app --host 127.0.0.1 --port 8000 --app-dir "%BACKEND_DIR%"
goto :eof
...
if errorlevel 1 (
    echo [错误] 未找到 python 命令。请确认 Python 3.11+ 已安装并加入 PATH。
    exit /b 1
)
...
set "PYTHON_MAJOR="
set "PYTHON_MINOR="
for /f "tokens=1,2 delims= " %%i in ('python --version 2^>^&1') do (
    set "PYTHON_PREFIX=%%i"
    set "PYTHON_VERSION=%%j"
)

if /i not "%PYTHON_PREFIX%"=="Python" (
    echo [错误] Python 版本输出异常：%PYTHON_PREFIX% %PYTHON_VERSION%
    exit /b 1
)
...
echo(%PYTHON_VERSION%| findstr /r "^[0-9][0-9]*\.[0-9][0-9]*" >nul
if errorlevel 1 (
    echo [错误] 无法解析 Python 版本：%PYTHON_VERSION%
    exit /b 1
)
...
if errorlevel 1 (
    echo [错误] Python 主版本解析失败：%PYTHON_VERSION%
    exit /b 1
)
echo(%PYTHON_MINOR%| findstr /r "^[0-9][0-9]*$" >nul
if errorlevel 1 (
    echo [错误] Python 次版本解析失败：%PYTHON_VERSION%
    exit /b 1
)
...
echo [run_local.bat] 检测到 Python %PYTHON_VERSION%
exit /b 0

:check_node
where node >nul 2>&1
if errorlevel 1 (
    echo [错误] 未找到 node 命令。请确认 Node.js 18+ 已安装并加入 PATH。
    exit /b 1
)
...
echo(%NODE_VERSION%| findstr /r "^[0-9][0-9]*\.[0-9][0-9]*" >nul
if errorlevel 1 (
    echo [错误] 无法解析 Node.js 版本：%NODE_VERSION%
    exit /b 1
)
...
if errorlevel 1 (
    echo [错误] Node.js 主版本解析失败：%NODE_VERSION%
    exit /b 1
)
...
echo [run_local.bat] 检测到 Node.js %NODE_VERSION%
exit /b 0

:check_npm
where npm >nul 2>&1
if errorlevel 1 (
    echo [错误] 未找到 npm 命令。请确认 Node.js 已完整安装并加入 PATH。
    exit /b 1
)
exit /b 0
...
where git >nul 2>&1
if errorlevel 1 (
    echo [run_local.bat] 未找到 Git，跳过自动更新。
    exit /b 0
)
